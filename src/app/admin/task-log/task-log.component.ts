...
export class TaskLogComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog,private dataProvider:DataProvider,private alertify:AlertsAndNotificationsService,private databaseService:DatabaseService) {}
  ...
  addTask(type:'todo' | 'ongoing' | 'completed'){
    ...
    dialogRef.componentInstance.addTask.subscribe((data:any)=>{
      ...
      if(data.type==='todo'){
        ...
        this.databaseService.addTodoTask(data.task).then(()=>{
        ...
        this.databaseService.addOnGoingTask(data.task).then(()=>{
          this.alertify.presentToast('Task added successfully');
        }).catch(()=>{
          this.alertify.presentToast('Error adding task');
          this.dataProvider.pageSetting.blur = false;
        ...
        this.databaseService.addCompletedTask(data.task).then(()=>{
          this.alertify.presentToast('Task added successfully');
        }).catch(()=>{
          this.alertify.presentToast('Error adding task');
          this.dataProvider.pageSetting.blur = false;
  ...
  changeStage(
    ...
    if (type === 'todoTasks') {
      ...
      this.databaseService.setTodoStage(this.todoTasks[index].id,event).then(()=>{
        this.dataProvider.pageSetting.blur = false;
      ...
      this.databaseService.setOnGoingStage(this.onGoingTasks[index].id,event).then(()=>{
        this.alertify.presentToast('Stage changed successfully');
        this.dataProvider.pageSetting.blur = false;
        this.ngOnInit();
      }).catch(()=>{
      ...
      this.databaseService.setCompletedStage(this.completedTasks[index].id,event).then(()=>{
        this.alertify.presentToast('Stage changed successfully');
        this.dataProvider.pageSetting.blur = false;
        this.ngOnInit();
      }).catch(()=>{
  ...
  changeAgent(
    ...
    if (type === 'todoTasks') {
      this.databaseService.updateTodoTask(this.todoTasks[index].id,{assignedAgentName:event.displayName,assignedAgentImage:event.photoURL,assignedAgentId:event.userId}).then(()=>{
      ...
      this.databaseService.updateOnGoingTask(this.onGoingTasks[index].id,{assignedAgentName:event.displayName,assignedAgentImage:event.photoURL,assignedAgentId:event.userId}).then(()=>{
        this.alertify.presentToast('Agent changed successfully');
        this.dataProvider.pageSetting.blur = false;
        this.ngOnInit();
      }).catch(()=>{
      ...
      this.databaseService.updateCompletedTask(this.completedTasks[index].id,{assignedAgentName:event.displayName,assignedAgentImage:event.photoURL,assignedAgentId:event.userId}).then(()=>{
        this.alertify.presentToast('Agent changed successfully');
        this.dataProvider.pageSetting.blur = false;
        this.ngOnInit();
      }).catch(()=>{
